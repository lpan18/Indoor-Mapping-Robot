...
public:
  ...
  /**
   * icp transform one step -- method for iteration use
   * @param point_mat1 nx2 matrix with points of laser scan 1 with a match in laser scan 2
   * @param point_mat2 nx2 matrix with points of laser scan 2 with a match in laser scan 1
   * @param rotation matrix R 2*2
   * @param translation vector T 2*1
   */
  static tf::Transform icpTransformOneStep(cv::Mat &point_mat1,
                                           cv::Mat &point_mat2);
