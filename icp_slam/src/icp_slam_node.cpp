...
ICPSlamNode::ICPSlamNode() : local_nh_("~")
{
  ...
  local_nh_.param("xmin", x_min_, -150.0);
  local_nh_.param("ymin", y_min_, -150.0);
  local_nh_.param("xmax", x_max_, 150.0);
  local_nh_.param("ymax", y_max_, 150.0);

  // TODO: initialize the map (width_, height_, origin_)
  width_ = (unsigned int)((x_max_ - x_min_) / resolution_);  // 6000 pixel
  height_ = (unsigned int)((y_max_ - y_min_) / resolution_); // 6000 pixel
...
void ICPSlamNode::laserCallback(const sensor_msgs::LaserScanConstPtr &laser_msg)
{
  ...
  try
  {
    tf_listener_.waitForTransform(odom_frame_id_, laser_msg->header.frame_id,
  ...
  tf_map_laser.frame_id_ = odom_frame_id_;
  tf_map_laser.child_frame_id_ = map_frame_id_;
  auto is_keyframe = icp_slam_->track(laser_msg, tf_odom_laser, tf_map_laser);

  if (is_keyframe)
  {
    tf_map_laser.stamp_ = ros::Time::now();
    // std::cout << "===In node=== " << tf_map_laser.getOrigin().getX() << " " << tf_map_laser.getOrigin().getY() << " " << tf_map_laser.getOrigin().getZ() << std::endl;
...
void ICPSlamNode::publishMap(ros::Time timestamp)
{
  ...
  cv::imwrite("/home/lei/cmpt742-fall2018-assignment3/tmp/map.png", map);
}
